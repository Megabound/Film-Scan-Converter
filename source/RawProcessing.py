...
class RawProcessing:
    ...
    advanced_attrs = [
        ...
        if key not in ("filetype", "frame", "fit_aspect_ratio")
    ]  # list of keys for advanced settings, except for keys that should not be saved
    ...
    def __init__(self, file_directory, default_settings, global_settings, config_path):
        ...
        self.proxy = (
            False  # Flag to keep track of whether or not proxies are being used
        )
        self.FileReadError = False  # flag for if the class could not read the RAW file
        self.active_processes = (
            0  # lets object know how many processes are currently processing the photo
        )
        self.pick_wb = (
            False  # flag to pick white balance from a set of pixel coordinates
        )
        ...
        try:  # to read in the parameters from a saved file
            directory = os.path.join(
                self.config_path, f"{self.filename.split('.')[0]}.npy"
            )
            ...
            for attr in self.processing_parameters:
                if attr in global_settings:
                    setattr(
                        self, attr, global_settings[attr]
                    )  # Initializes every instance parameter based on default value
                else:
            ...
            for attr in self.processing_parameters:
                if attr in params_dict:
                    setattr(
                        self, attr, params_dict[attr]
                    )  # Initializes every instance parameter with imported parameters
                elif attr in default_settings:
                    setattr(
                        self, attr, default_settings[attr]
    ...
    def load(self, full_res=False):
        ...
        try:
            with rawpy.imread(self.file_directory) as raw:  # tries to read as raw file
                self.RAW_IMG = raw.postprocess(
                    ...
                    use_camera_wb=self.class_parameters[
                        "use_camera_wb"
                    ],  # Screws up the colours if not used
                    user_wb=self.class_parameters["wb_mult"],  # wb multipliers
                    demosaic_algorithm=rawpy.DemosaicAlgorithm(
                        self.class_parameters["dm_alg"]
                    ),
                    fbdd_noise_reduction=rawpy.FBDDNoiseReductionMode(
                        self.class_parameters["fbdd_nr"]
                    ),
                    output_color=rawpy.ColorSpace(
                        self.class_parameters["colour_space"]
                    ),
                ...
                self.colour_desc = raw.color_desc.decode(
                    "utf-8"
                )  # get the bayer pattern
        except Exception as _:
            try:
                self.RAW_IMG = cv2.imread(
                    self.file_directory, cv2.IMREAD_UNCHANGED
                )  # if fails, reads as normal image
                if type(self.RAW_IMG) is not np.ndarray:
                    raise Exception(f"{self.file_directory} failed to load!")
            except Exception as e:
                logger.exception(
                    f"Exception: {e}"
                )  # If fails again, set error attributes
                self.reject = True
                self.FileReadError = True
                return
            else:
                if self.RAW_IMG.dtype == np.uint8:
                    self.RAW_IMG = (self.RAW_IMG).astype(
                        np.uint16, copy=False
                    ) * 256  # converts image to 16-bit
        else:
            # RawPy sometimes misreads the RAW image size, this removes any extra black borders
            gray = cv2.cvtColor(
                cv2.convertScaleAbs(self.RAW_IMG, alpha=(255.0 / 65535.0)),
                cv2.COLOR_RGB2GRAY,
            )
        ...
        self.memory_alloc = (
            self.RAW_IMG.nbytes * 4 * 12
        )  # estimation of memory requirements based on the size of the image
    ...
    def get_IMG(self, output=None, as_array=False):
        ...
        match output:
            ...
            case "Contours":  # generate contour image, then return it
                ...
                zebra = np.repeat(
                    (np.mod(indices[0] + indices[1], zebra_width * 2) > zebra_width)[
                        :, :, np.newaxis
                    ],
                    3,
                    axis=2,
                ...
                thresh_img = np.where(
                    zebra, 0, thresh_img
                )  # applies zebra pattern to threshold image
                img = cv2.addWeighted(
                    cv2.convertScaleAbs(self.RAW_IMG, alpha=(255.0 / 65535.0)),
                    1,
                    thresh_img,
                    0.2,
                    0,
                )  # add threshold image to RAW

                # drawing crop boxes
                if self.rect is not None:
                    border_width = np.ceil(
                        (img.shape[0] + img.shape[1]) / 800
                    )  # border width proportional to image size
                    ...
                    rect = (
                        (self.rect[0][0] * y, self.rect[0][1] * x),
                        (self.rect[1][0] * y, self.rect[1][1] * x),
                        self.rect[2],
                    )
                    ...
                    if (
                        self.border_crop < 0
                        and self.class_parameters["ignore_neg_border"]
                    ):
                        EQ_ignore_box = self.shrink_box(EQ_ignore_box, -x_crop, -y_crop)
                    EQ_ignore_box = self.shrink_box(
                        EQ_ignore_box, ignore_border[0], ignore_border[1]
                    )
                    ...
                    img = np.where(
                        np.dstack([np.sum(EQ_ignore_poly, (2)) == 0] * 3),
                        img,
                        cv2.addWeighted(EQ_ignore_poly, 2, img, 0.8, 0),
                    )  # shaded zone where EQ calcs are ignored

                    cv2.drawContours(
                        img, [box], 0, (0, 255, 255), int(border_width * 0.75)
                    )  # original crop
                    cv2.drawContours(
                        img,
                        ...
                        -1,
                        (0, 255, 255),
                        int(border_width * 0.75),
                    )  # largest contour
                    cv2.drawContours(
                        img, [extra_crop_box], 0, (0, 255, 0), int(border_width)
                    )  # extra border crop
                img = self.rotate(img)  # apply rotation to image
        ...
        match self.class_parameters["filetype"]:
            case "JPG":
                quality = [
                    ...
                    self.class_parameters["jpg_quality"],
                ]
                cv2.imwrite(
                    filename, cv2.convertScaleAbs(img, alpha=(255.0 / 65535.0)), quality
                )  # Must convert to 8-bit image before exporting as JPG
            case "TIFF":
                quality = [
                    cv2.IMWRITE_TIFF_COMPRESSION,
                    self.class_parameters["tiff_compression"],
                ]
    ...
    def save_settings(self):
        ...
        directory = os.path.join(
            self.config_path, f"{self.filename.split('.')[0]}.npy"
        )  # uses the same name as the input file
        params_dict = dict()
        for attr in self.processing_parameters:
    ...
    def process(self, full_res=False, recent_only=False, skip_crop=False):
        ...
        self.active_processes += (
            1  # Used to keep track of the number of processes currently running
        )
        ...
        if not skip_crop or not hasattr(self, "thresh"):
            ...
            if (
                img_size > self.class_parameters["max_proxy_size"]
            ):  # Checks if image is larger than the allowable size, if yes, then generate proxy images to speed up preview generation
    ...
    def colour_negative_processing(self, img):
        img = 65535 - img  # invert to create positive image
        img = self.slide_processing(
            img
        )  # The rest of the processing is identical to slide_processing
        return img
    ...
    def slide_processing(self, img):
        ...
        wb_mode = [
            self.wb_adjust,
            self.wb_adjust_coeff,
            self.wb_adjust_gamma,
        ]  # different ways to adjust wb, for debugging
    ...
    def find_dust(self, img):
        ...
        x, y = (
            np.array(self.class_parameters["ignore_border"]) / 100 * img.shape[:2][::-1]
        ).astype(np.int32)  # calculates the width of the border to ignore in pixels
        if x * y == 0:
            sample = np.s_[:]
        else:
        ...
        threshold = (maximum - minimum) * self.class_parameters[
            "dust_threshold"
        ] / 100 + minimum
        _, thresh = cv2.threshold(imgray, threshold, 255, cv2.THRESH_BINARY_INV)
        thresh_img = cv2.dilate(
            thresh, kernel, iterations=self.class_parameters["dust_iter"]
        )
        thresh_img = cv2.erode(
            thresh_img, kernel, iterations=self.class_parameters["dust_iter"]
        )
    ...
    def find_optimal_crop(self):
        ...
        if rect[2] <= 0:
            rect = (
                ...
                (rect[1][1], rect[1][0]),
                rect[2] + 90,
            )  # correction for if the rectangle rotation is exactly zero
        rect = (
            (rect[0][0] / y, rect[0][1] / x),
            (rect[1][0] / y, rect[1][1] / x),
            rect[2],
        )  # normalizes crop for different sized images
    ...
    def crop(self, img, rect, include_EQ_ignore=False):
        if rect is not None:
            ...
            rect = (
                ...
                (rect[1][0] * y, rect[1][1] * x),
                rect[2],
            )  # convert normalized crop to corresponding pixel coordinates
            box = cv2.boxPoints(rect)
            box = np.int64(box)
            ...
            dst_pts = np.array(
                [[0, width - 1], [0, 0], [height - 1, 0], [height - 1, width - 1]],
                dtype="float32",
            )
            ...
            if rect[2] > 45:
                ...
                )  # rotates image back if it has been rotated
        return img
    ...
    def get_threshold(self, img):
        ...
        _, light_thresh_img = cv2.threshold(
            imgray, light_threshold, 255, cv2.THRESH_BINARY_INV
        )
    ...
    def hist_EQ(self, img):
        ...
        if self.base_detect and (self.film_type == 1 or self.film_type == 2):
            ...
            black_point = np.percentile(
                sample_img, self.class_parameters["black_point_percentile"], (0, 1)
            )
        ...
        white_point = np.percentile(
            sample_img, self.class_parameters["white_point_percentile"], (0, 1)
        )
        white_multipliers = np.divide(
            65535 + self.white_point / 100 * sensitivity * 65535,
            white_point,
            out=max_array,
            where=white_point > 0,
        )  # division, but ignore divide by zero or negative
        img = np.multiply(
            img, white_multipliers
        )  # Scales the white percentile to 65535
        return img
    ...
    def wb_adjust(self, img):
        ...
        if self.pick_wb:  # logic to calculated temp and tint values from the wb picker
            self.pick_wb = False
            x, y, r = (
                self.wb_picker_params
            )  # unpacks parameters passed in from white balance picker
            wb_mask = self.rotate(
                np.zeros_like(self.crop(img, self.rect)[:, :, 0], dtype=np.uint8)
            )  # generate blank mask, rotate to same orientation as preview image
            ...
            wb_mask = cv2.circle(
                wb_mask, (x, y), radius, 255, -1
            )  # generate small circle to average pixels with
            wb_mask = self.rotate(
                wb_mask, True
            )  # rotate image back to default orientation
            meanBGR = cv2.mean(
                self.crop(img, self.rect), wb_mask
            )  # returns BGR tuple containing average of unmasked pixels
        ...
        adjustment = np.array(
            [-self.temp * multiplier, -self.tint * multiplier, self.temp * multiplier],
            np.float32,
        )
    ...
    def wb_adjust_coeff(self, img):
        ...
        if self.pick_wb:  # logic to calculated temp and tint values from the wb picker
            ...
            self.temp = max(
                min(
                    ((2 * G - (2 * G + R) * self.tint / multiplier) / 2 / R - 1)
                    * multiplier,
                    100,
                ),
                -100,
        ...
        img = np.multiply(
            ...
            np.array(
                [
                    1 - self.temp / multiplier + self.tint / multiplier / 2,
                    1 - self.tint / multiplier,
                    1 + self.temp / multiplier + self.tint / multiplier / 2,
                ]
            ),
        )
    ...
    def wb_adjust_gamma(self, img):
        ...
        img = np.power(
            img,
            (2 ** np.array([self.temp / 100, self.tint / 100, -self.temp / 100])),
            dtype=np.float32,
        )
    ...
    def exposure(self, img):
        ...
        img = (img ** (2 ** (-self.gamma / 100))).astype(
            np.float32, copy=False
        )  # gamma adjustment via gamma correction
        ...
        highlights_coefficient = (
            -4.15e-5 * self.highlights**2 + 0.02185 * self.highlights
        )
    ...
    def sat_adjust(self, img):
        ...
        norm = matplotlib.colors.Normalize(0, 65535, True)
        img = norm(np.flip(img, 2)).astype(
            np.float32, copy=False
        )  # Convert from bgr to rgb, and normalize input to (0,1)
        img = matplotlib.colors.rgb_to_hsv(img)  # Convert from rgb to hsv
        img[:, :, 1] = np.clip(
            img[:, :, 1] * sat_adjust, 0, 1
    ...
    def draw_histogram(self, img):
        ...
        hist_plot = np.zeros(
            self.class_parameters["histogram_plt_size"], np.uint8
        )  # Base image for the histogram
        width = self.class_parameters["histogram_plt_size"][1]
        height = (
            self.class_parameters["histogram_plt_size"][0] - 10
        )  # Leave a little border from the top
        histograms = []
        maxes = []
        ...
        for channel in channels:
            hist = cv2.calcHist(
                [channel], [0], None, bins, [0, 65536]
            )  # Generates the histogram
            maxes.append(
                np.max(hist)
            )  # Keeps track of the maximum value of the histogram
            hist[1:-1] = cv2.GaussianBlur(
                hist[1:-1], (smoothing, smoothing), 0
            )  # Smooths out the histogram
        ...
        for hist, colour in zip(histograms, colours):
            if np.max(maxes) != 0:
                hist = (
                    hist / np.max(maxes) * height
                )  # Scales all histograms to fit in the image
            pts = (
                np.stack((np.linspace(0, width, len(hist)), hist), -1)
                .reshape(-1, 1, 2)
                .squeeze()
                .tolist()
            )  # Reformats hist into a list of 2d points
            pts.insert(0, [0, 0])
            pts.append([width, 0])
            new_plot = np.zeros(self.class_parameters["histogram_plt_size"], np.uint8)
            new_plot = cv2.fillPoly(
                new_plot, np.array([pts]).astype(np.int32), color=colour
            )  # Generates histogram as a polygon
            hist_plot = (
                hist_plot + new_plot
            )  # Add current histogram channel to the overall histogram plot
        return hist_plot
    ...
    def add_frame(self, img):
        ...
        frame_size = max(
            1, int(min(img.shape[:2]) * self.class_parameters["frame"] / 100)
        )  # at least one pixel wide frame
        new_shape = (
            img.shape[0] + 2 * frame_size,
            img.shape[1] + 2 * frame_size,
        ) + img.shape[2:]
        frame_img = np.ones(new_shape, dtype=img.dtype) * 65535
        frame_img[frame_size:-frame_size, frame_size:-frame_size] = (
            img  # center image inside frame
        )

        if (
            self.class_parameters["fit_aspect_ratio"] != "Keep Original"
        ):  # fit image to aspect ratio
            target_w, target_h = map(
                int,
                self.class_parameters["fit_aspect_ratio"].split(" ", 1)[0].split(":"),
            )  # parse 'W:H (text)' to W, H
            ...
            if current_ratio > target_ratio:  # image is wider than target aspect ratio
                ...
                padding = (
                    (pad, pad + (new_height - frame_img.shape[0]) % 2),
                    (0, 0),
                ...
                padding = (
                    (0, 0),
                    (pad, pad + (new_width - frame_img.shape[1]) % 2),
                ) + (() if img.ndim == 2 else ((0, 0),))
            frame_img = np.pad(
                frame_img, padding, mode="constant", constant_values=65535
            )
    ...
    def set_wb_from_picker(self, x, y):
        ...
        self.pick_wb = (
            True  # sets flag to measure and set white balance on next processing
        )
        self.process()
    ...
    def get_base_colour(self, x, y):
        ...
        base_mask = self.rotate(
            np.zeros_like(self.RAW_IMG[:, :, 0], dtype=np.uint8)
        )  # generate blank mask, rotate to same orientation as preview image
        # applying scale factors based on image size
        x = int(x * base_mask.shape[1])
        y = int(y * base_mask.shape[0])
        radius = int(
            min(base_mask.shape) * self.class_parameters["picker_radius"] / 100
        )

        base_mask = cv2.circle(
            base_mask, (x, y), radius, 255, -1
        )  # generate small circle to average pixels with
        base_mask = self.rotate(
            base_mask, True
        )  # rotate image back to default orientation
        raw = cv2.convertScaleAbs(self.RAW_IMG, alpha=(255.0 / 65535.0))
        meanBGR = cv2.mean(raw, base_mask)[
            :-1
        ]  # returns BGR tuple containing average of unmasked pixels
