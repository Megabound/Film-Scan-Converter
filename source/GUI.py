...
class GUI:
    def __init__(self, master, output_directory=None):
        ...
        self.in_progress = (
            set()
        )  # keeps track photos that are in processing when first loading
        self.photo_process_values = [
            "RAW",
            "Threshold",
            "Contours",
            "Histogram",
        ...
        self.allowable_image_filetypes = [
            (
                "RAW files",
                "*.DNG *.CR2 *.CR3 *.NEF *.ARW *.RAF *.ORF *.ORI *.ERF *.GPR *.RAW *.CRW *.RW2 *.dng *.cr2 *.cr3 *.nef *.arw *.raf *.orf *.ori *.erf *.grp *.raw *.crw *.rw2",
            ),
            (
                "Image files",
                "*.PNG *.JPG *.JPEG *.BMP *.TIFF *.TIF *.png *.jpg *.jpeg *.bmp *.tiff *.tif",
            ),
        ]
        self.header_style = (
            "Segoe UI",
            10,
            "normal",
        )  # Defines font for section headers
        ...
        self.filemenu.add_command(
            label="Save Settings", command=self.save_settings, accelerator="Ctrl+S"
        )
        ...
        self.editmenu.add_command(
            label="Copy Settings", command=self.copy_settings, accelerator="Ctrl+C"
        )
        self.editmenu.add_command(
            label="Paste Settings",
            command=self.paste_settings,
            state=tk.DISABLED,
            accelerator="Ctrl+V",
        )
        self.editmenu.add_separator()
        self.editmenu.add_command(
            label="Reset to Default Settings", command=self.reset_settings
        )
        self.editmenu.add_separator()
        self.editmenu.add_command(
            label="Advanced Settings...", command=self.advanced_dialog
        )
        ...
        importFrame = ttk.LabelFrame(
            dynamic_scroll_frame.frame,
            borderwidth=2,
            labelwidget=import_title,
            padding=5,
        )
        ...
        self.import_button = ttk.Button(
            importSubFrame1, text="Import...", command=self.import_photos, width=8
        )
        ...
        self.prevButton = ttk.Button(
            importSubFrame2, text="< Previous Photo", width=20, command=self.previous
        )
        self.prevButton.pack(side=tk.LEFT, padx=2, pady=5)
        self.set_tooltip(self.prevButton, "(Left Arrow)")
        self.nextButton = ttk.Button(
            importSubFrame2, text="Next Photo >", width=20, command=self.next
        )
        self.nextButton.pack(side=tk.LEFT, padx=2, pady=5)
        self.set_tooltip(self.nextButton, "(Right Arrow)")

        # Processing Frame
        processing_title = ttk.Label(
            text="Processing Settings", font=self.header_style, padding=2
        )
        processingFrame = ttk.LabelFrame(
            dynamic_scroll_frame.frame,
            borderwidth=2,
            labelwidget=processing_title,
            padding=5,
        )
        ...
        film_types = [
            ...
            "Colour Negative",
            "Slide (Colour Positive)",
            "Crop Only (RAW)",
        ]
        self.film_type = ComboLabel(
            self.filmFrame,
            "Film Type:",
            0,
            film_types,
        ...
        controls_title = ttk.Label(
            text="Automatic Crop & Rotate", font=self.header_style, padding=2
        )
        self.cropFrame = ttk.LabelFrame(
            dynamic_scroll_frame.frame,
            borderwidth=2,
            labelwidget=controls_title,
            padding=5,
        )
        ...
        self.flip_check = CheckLabel(
            crop_adjustments,
            "Flip Horizontally:",
            3,
            "flip",
            self.widgets,
            command=self.set_flip,
        )
        ...
        ttk.Button(
            ...
            width=22,
            command=self.rot_counterclockwise,
        ).pack(side=tk.LEFT, padx=2, pady=5)
        self.set_tooltip(rotButtons.winfo_children()[-1], "(Shift+R)")
        ttk.Button(
            rotButtons, text="Rotate Clockwise", width=22, command=self.rot_clockwise
        ).pack(side=tk.LEFT, padx=2, pady=5)
        ...
        self.colourFrame = ttk.LabelFrame(
            dynamic_scroll_frame.frame,
            borderwidth=2,
            labelwidget=colour_title,
            padding=5,
        )
        ...
        self.rgb_display = tk.Frame(
            colour_controls,
            bg=self._from_rgb(self.global_settings["base_rgb"]),
            height=20,
            width=20,
            relief=tk.GROOVE,
            borderwidth=1,
        )
        self.base_pick_button = ttk.Button(
            colour_controls, image=picker, command=self.set_base
        )
        self.base_pick_button.image = picker
        self.base_buttons_frame = ttk.Frame(colour_controls)
        ttk.Button(
            self.base_buttons_frame,
            text="Set RGB",
            command=lambda: self.set_base_rgb(0),
            width=8,
        ).pack(side=tk.LEFT, padx=2, pady=2, anchor="center")
        ttk.Button(
            self.base_buttons_frame,
            text="Import Blank...",
            command=lambda: self.set_base_rgb(2),
            width=13,
        ).pack(side=tk.LEFT, padx=2, pady=2, anchor="center")
        ttk.Label(colour_controls, text="White Balance Picker:").grid(
            row=3, column=0, sticky=tk.E
        )
        self.wb_picker_button = ttk.Button(
            colour_controls, image=picker, command=self.pick_wb
        )
        self.wb_picker_button.grid(row=3, column=1, sticky=tk.W)
        self.wb_picker_button.image = picker
        self.temp = ScaleEntry(
            colour_controls,
            "Temperature:",
            4,
            -100,
            ...
            self.widgets,
            increment=1,
            command=lambda widget: self.widget_changed(widget, "skip crop"),
        )
        self.tint = ScaleEntry(
            colour_controls,
            "Tint:",
            5,
            -100,
        ...
        # Brightness Settings
        brightness_title = ttk.Label(
            text="Brightness Adjustment", font=self.header_style, padding=2
        )
        self.exposureFrame = ttk.LabelFrame(
            dynamic_scroll_frame.frame,
            borderwidth=2,
            labelwidget=brightness_title,
            padding=5,
        )
        ...
        export_title = ttk.Label(
            text="Export Settings", font=self.header_style, padding=2
        )
        export_frame = ttk.LabelFrame(
            dynamic_scroll_frame.frame,
            borderwidth=2,
            labelwidget=export_title,
            padding=5,
        )
        ...
        ComboLabel(
            export_settings_frame,
            "Export File Type:",
            0,
            self.filetypes,
            "filetype",
            global_sync=False,
            output_list=self.filetypes,
            command=lambda widget: self.widget_changed(widget, "skip", False),
            default_value=RawProcessing.default_parameters["filetype"],
        )
        self.frame = ScaleEntry(
            export_settings_frame,
            "White Frame (%):",
            1,
            0,
            ...
            "frame",
            global_sync=False,
            command=lambda widget: self.widget_changed(widget, "update", False),
            default_value=RawProcessing.default_parameters["frame"],
        )
        ComboLabel(
            export_settings_frame,
            "Fit Aspect Ratio:",
            ...
            "fit_aspect_ratio",
            global_sync=False,
            output_list=self.fit_aspect_ratios,
            command=lambda widget: self.widget_changed(widget, "update", False),
            default_value=RawProcessing.default_parameters["fit_aspect_ratio"],
            width=20,
        )
        export_settings_frame.pack(fill="x", pady=(0, 15))  # adds some spacing

        ttk.Label(export_frame, text="Output Destination Folder:", anchor="w").pack(
            fill="x"
        )
        self.destination_folder_text = tk.StringVar()
        self.destination_folder_text.set("No Destination Folder Specified")
        destination_lbl = ttk.Label(
            export_frame,
        ...
        destination_lbl.bind(
            "<Configure>",
            lambda e: destination_lbl.config(wraplength=destination_lbl.winfo_width()),
        )
        ttk.Button(export_frame, text="Select Folder", command=self.select_folder).pack(
        ...
        self.abort_button = ttk.Button(
            export_frame, text="Abort Export", command=self.abort
        )
        ...
        self.read_error_lbl = ttk.Label(
            mainFrame,
            text="Error: File could not be read",
            font=("Segoe UI", 20),
            justify="center",
            anchor="center",
        )
        self.read_error_lbl.bind(
            "<Configure>",
            lambda e: self.read_error_lbl.config(
                wraplength=self.read_error_lbl.winfo_width()
            ),
        )
        ...
        self.photo_process_Combo = ttk.Combobox(
            process_select_Frame, state="readonly", values=self.photo_process_values
        )
        ...
        # Converted Preview
        ttk.Label(self.outputFrame, text="Preview:", font=self.header_style).grid(
            row=2, column=0
        )
        ...
        try:
            params_dict = np.load(
                os.path.join(self.config_path, "config.npy"), allow_pickle=True
            ).item()
            ...
            for settings in [self.advanced_settings, RawProcessing.class_parameters]:
                for attr in settings:
                    if attr in params_dict:
                        settings[attr] = params_dict[
                            attr
                        ]  # Initializes every parameter with imported parameters
                    else:
                        logger.exception(
                            f"Attribute {attr} not found in imported config.npy file."
                        )
    ...
    def advanced_dialog(self):
        ...
        import_settings = ttk.LabelFrame(
            firstColumn, borderwidth=2, labelwidget=import_lbl, padding=5
        )
        ...
        export_settings = ttk.LabelFrame(
            firstColumn, borderwidth=2, labelwidget=export_lbl, padding=5
        )
        ...
        process_lbl = ttk.Label(
            top, text="Processing:", font=self.header_style, padding=2
        )
        process_settings = ttk.LabelFrame(
            secondColumn, borderwidth=2, labelwidget=process_lbl, padding=5
        )
        process_settings.pack(fill="x", expand=True)
        dust_lbl = ttk.Label(
            top, text="Dust Removal:", font=self.header_style, padding=2
        )
        dust_settings = ttk.LabelFrame(
            secondColumn, borderwidth=2, labelwidget=dust_lbl, padding=5
        )
        ...
        MultiEntryLabel(
            ...
            10,
            1,
            key="median_filter_passes",
            widget_dictionary=advanced_widgets,
            width=25,
        )
        cs_list = (
            "raw",
        ...
        ComboLabel(
            ...
            advanced_widgets,
            False,
            output_list=tiff_comp_out,
            width=20,
        )
        MultiEntryLabel(
            export_settings,
            "Max Processors Override:",
            2,
            0,
            multiprocessing.cpu_count(),
            1,
            key="max_processors_override",
            widget_dictionary=advanced_widgets,
        )
        ...
        ttk.Button(buttonFrame, text="Cancel", command=quit).pack(
            side=tk.RIGHT, padx=2, pady=5, anchor="sw"
        )
        ttk.Button(buttonFrame, text="Save", command=save_settings).pack(
            side=tk.RIGHT, padx=2, pady=5, anchor="sw"
        )
        ttk.Button(buttonFrame, text="Reset to Default", command=reset, width=17).pack(
            side=tk.RIGHT, padx=2, pady=5, anchor="sw"
        )

        # Centres pop-up window over self.master window
        top.update_idletasks()
        x = self.master.winfo_x() + int(
            (self.master.winfo_width() / 2) - (top.winfo_width() / 2)
        )
        y = self.master.winfo_y() + int(
            (self.master.winfo_height() / 2) - (top.winfo_height() / 2)
        )
    ...
    def widget_changed(
        self,
        widget,
        mode: Literal["normal", "skip crop", "update", "skip"] = "normal",
        instance=True,
    ):
    ...
    def import_photos(self):
        ...
        filenames = filedialog.askopenfilenames(
            title="Select RAW File(s)", filetypes=self.allowable_image_filetypes
    ...
    def load_IMG(self, event=None):
        ...
        for i, photo in enumerate(self.photos):
            if (
                (abs(i - photo_index) <= self.advanced_settings["preload"])
                and not hasattr(photo, "RAW_IMG")
                and i not in self.in_progress
            ):  # preload photos in buffer ahead or behind of the currently selected one
                threading.Thread(
                    target=load_async, args=(photo, i), daemon=True
                ).start()
                self.in_progress.add(i)  # keeps track of photos in progress
            elif (abs(i - photo_index) > self.advanced_settings["preload"]) and hasattr(
                photo, "RAW_IMG"
            ):  # delete photos outside of buffer
                photo.clear_memory()
    ...
    def update_IMG(self, full_res=True):
            ...
            def check_if_done(t):
                if not t.is_alive():
                    if not self.current_photo.active_processes:
                        self.update_IMG(
                            False
                        )  # update image, but do not queue full res process again
                else:
                    self.master.after(
                        300, check_if_done, t
                    )  # if not done, wait some time, then check again

            t = threading.Thread(
                target=self.current_photo.process, args=[True, True, True], daemon=True
            )  # Thread to generate full res previews
            t.start()
        ...
        try:
            ...
            self.read_error_lbl.grid(
                row=0, column=1, sticky="EW"
            )  # displays error message when image cannot be loaded
            return
        ...
        if self.photo_process_Combo.current() == 4:  # if "Full Preview" is selected
            ...
            self.photo_display_height = max(
                int(self.master.winfo_height() - 100), 100
            )  # resize image to full display height
        else:
            self.photo_display_height = max(
                int((self.master.winfo_height() - 100) / 2), 100
            )
            process_photo = ImageTk.PhotoImage(
                self.resize_IMG(
                    self.current_photo.get_IMG(
                        self.photo_process_values[self.photo_process_Combo.current()]
                    )
                )
            )
        ...
        if full_res:
            ...
            if (
                self.photo_process_Combo.current() == 4
            ):  # Only display when full preview is selected
                try:
                    self.master.after_cancel(self.start_full_res)
                except:
                    pass
                finally:
                    self.start_full_res = self.master.after(
                        500, update_full_res
                    )  # waits for 0.5 s of inactivity before processing
    ...
    def resize_UI(self):
        ...
        self.photo_display_width = max(
            int(self.master.winfo_width() - self.controlsFrame.winfo_width() - 20), 100
        )
    ...
    def reset_settings(self):
        ...
        if self.glob_check.get():
            affected = sum(
                [photo.use_global_settings for photo in self.photos]
            )  # calculate the total number of photos using global settings
            if affected > 1:
                if not messagebox.askyesno(
                    "Reset to Default Settings",
                    f"You are about to globally reset {str(affected)} photos's settings.\nDo you wish to continue?",
                    icon="warning",
                ):
                    return
            ...
            self.changed_global_settings()
        self.apply_settings(
            self.current_photo, self.default_settings
        )  # apply new settings to photo
    ...
    def update_UI(self):
        ...
        if (
            (self.film_type.get() == 1) or (self.film_type.get() == 2)
        ) and not self.reject_check.get():
            self.colourFrame.grid(
                row=6, column=0, sticky="EW"
            )  # Show the colour frame only for colour and slides
        else:
        ...
        if self.glob_check.get():
            ...
            if affected > 1:
                if messagebox.askyesno(
                    "Flip Photo",
                    f"Do you want to globally flip {str(affected)} photos?",
                    default="no",
                ):
    ...
    def pick_wb(self):
        ...
        self.result_photo.configure(
            cursor="tcross"
        )  # changes the cursor over the preview to a cross
        self.result_photo_frame.configure(
            background="red"
        )  # highlights the preview image
    ...
    def click(self, event):
        ...
        if self.wb_picker and (event.widget != self.wb_picker_button):
            ...
            if event.widget == self.result_photo:
                ...
                self.current_photo.set_wb_from_picker(
                    x, y
                )  # set the white balance to neutral at the x, y coordinates of the mouse click
                ...
                if self.glob_check.get():
                    ...
                    if affected > 1:
                        if messagebox.askyesno(
                            ...
                            f"Do you want to apply this white balance adjustment globally to {str(affected)} photos?",
                            default="no",
                        ):
            ...
            self.process_photo_frame.configure(
                background=self.master.cget("bg")
            )  # reset frane
            self.base_picker = False
            if event.widget == self.process_photo:
                x = event.x / event.widget.winfo_width()
                y = event.y / event.widget.winfo_height()
                self.current_photo.get_base_colour(
                    x, y
                )  # retrieve base colour at the x, y coordinates of the mouse click
                self.set_base_rgb(1)
    ...
    def set_base_detect(self, event=None):
        ...
        if self.glob_check.get():
            ...
            if affected > 1:
                if messagebox.askyesno(
                    "Base Colour Changed",
                    f"Do you want change the base colour globally to {str(affected)} photos?",
                    default="no",
                ):
            ...
            case 2:  # pick colour from blank scan
                ...
                if blank.FileReadError:
                    messagebox.showerror(
                        "Read Blank",
                        "RAW image could not be read. Verify the integrity of the RAW image.",
                    )
                raw_img = cv2.convertScaleAbs(blank.RAW_IMG, alpha=(255.0 / 65535.0))[
                    :, :, ::-1
                ]
                brightness = np.sum(raw_img.astype(np.uint16), 2)
                sample = np.percentile(
                    brightness, 90
                )  # take sample at 90th percentile brightest pixel
    ...
    def export_individual(self):
        ...
        filename = os.path.join(
            self.destination_folder, str(self.current_photo).split(".")[0]
    ...
    def export_multiple(self):
        ...
        has_alloc = (
            0  # number of photos in which the memory allocation has been calculated
        )
        with multiprocessing.Manager() as manager:
            self.terminate = (
                manager.Event()
            )  # flag to abort export and safely close processes
            ...
            for photo in self.photos:
                ...
                if photo.use_global_settings:
                    self.apply_settings(
                        photo, self.global_settings
                    )  # Ensures the proper settings have been applied
                filename = os.path.join(
                    self.destination_folder, str(photo).split(".")[0]
                )  # removes the file extension
                inputs.append(
                    (photo, filename, self.terminate, RawProcessing.class_parameters)
                )
                if hasattr(photo, "memory_alloc"):
                    allocated += (
                        photo.memory_alloc
                    )  # tally of estimated memory requirements of each photo
                    has_alloc += 1
            ...
            if self.advanced_settings["max_processors_override"] != 0:
                ...
                allocated = (
                    allocated / has_alloc
                )  # allocated memory as average of estimated memory requirements for each photo
                # print(round(allocated / 1e9,1),'GB allocated')
                max_processors = round(available / allocated)
            processes = max(
                min(max_processors, multiprocessing.cpu_count(), len(inputs)), 1
            )  # allocates number of processors between 1 and the maximum number of processors available
            # print(processes, 'processes allocated for export')

            self.update_progress(
                20, "Allocating " + str(processes) + " processor(s) for export..."
            )
            with multiprocessing.Pool(processes) as self.pool:
                ...
                for result in self.pool.imap(self.export_async, inputs):
                    ...
                    self.update_progress(
                        i / len(inputs) * 80 + 19.99, update_message
    ...
    def key_handler(self, event):
        ...
        match event.keysym:
            ...
            case "f":
                ...
                if current_view == 4:
                    self.photo_process_Combo.current(
                        self.previous_view
                    )  # Return to previous
                else:
    ...
    def set_tooltip(self, widget, text, delay=500):
        ...
        def show_tooltip():
            tooltip.place(
                x=widget.winfo_rootx() - self.master.winfo_rootx() + 10,
    ...
    def on_closing(self):
        ...
        if len(self.photos) > 0:
            if self.ask_save_settings() is None:  # if "Cancel" is pressed, do nothing
                return
        if (
            hasattr(self, "export_thread") and self.export_thread.is_alive()
        ):  # check if the export thread is still alive
            if messagebox.askyesno(
                title="Export in Progress",
                icon="warning",
                message="Export is still in progress. Do you really want to quit?",
                default="no",
            ):
                try:
                    self.pool.terminate()
    ...
    def ask_save_settings(self):
        ...
        if self.unsaved:
            result = messagebox.askyesnocancel(
                "Unsaved Changes",
                "Do you want to save the changes you made to this batch of photos?",
            )
        else:
            return False
        if result:
    ...
    def save_settings(self, event=None):
        ...
        for photo in self.photos:
            if photo.use_global_settings:
                self.apply_settings(
                    photo, self.global_settings
                )  # apply most current settings before saving
            photo.save_settings()
        self.unsaved = False
        ...
        if hasattr(self, "copied_settings") and hasattr(self, "current_photo"):
            ...
            if self.current_photo.use_global_settings:
                ...
                if affected > 1:
                    if messagebox.askyesno(
                        "Paste Settings",
                        f"Do you want to paste the settings globally to {str(affected)} photos?",
                    ):
